...
class ArrayDatatableResultTest extends WebTestCase
{
    use ResetDatabase;
    ...
    public function testDatatableResponseWithoutData()
    {
        ...
        $client->request('GET', '/test-post/empty');

        $response = $client->getResponse();

        $this->assertSame(200,$response->getStatusCode());
        $this->assertSelectorExists('#datatable', $client->getResponse()->getContent());
        $element = $client->getCrawler()->filter('#datatable');
        $this->assertSame('{"options":{"columns":[{"data":"Name"},{"data":"Position"},{"data":"Office"},{"data":"Extn"},{"data":"Start date"},{"data":"Salary"}],"columnDefs":[{"targets":0,"visible":true,"orderable":true},{"targets":1,"visible":true,"orderable":true},{"targets":2,"visible":true,"orderable":true},{"targets":3,"visible":true,"orderable":true},{"targets":4,"visible":true,"orderable":true},{"targets":5,"visible":true,"orderable":true}],"language":{"url":"\/\/cdn.datatables.net\/plug-ins\/1.10.15\/i18n\/English.json"},"data":[]}}', $element->attr('data-aziz403--ux-datatable--datatable-view-value'));
    }

    public function testDatatableResponseWithData()
    {
        $client = static::createClient();
        $client->request('GET', '/test-post');
